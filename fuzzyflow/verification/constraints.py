...
# License. For details, see the LICENSE file.import math
...
from dace.transformation.interstate import loop_detection
...
class LoopDetection(loop_detection.DetectLoop, MultiStateTransformation):
...
def constrain_symbols(
    ...
    for k, v in ct.arrays.items():
        n_dims = len(v.shape)
        max_dim_size = min(math.floor((1e9 / 8) ** (1 / n_dims)), max_dim)
        for s in v.shape:
